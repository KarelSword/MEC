...
def core(points_set, degree, points, adjacency_matrix):
    ...
    while True:
        ...
        """
        p_prio, chosen_point_ne = include_r_point(chosen_point_ne, center, points_set, mbs_set)
        for value in p_prio:
        ...
        chosen_point_ne = sort_distance(chosen_point_ne, center, points_set)
        """
        # 读取MBS中点位置信息
        ...
        for candidate in chosen_point_ne:
            ...
            """
            try:
                mbs_set[str(points_set[str(candidate)].belongs_to)]
            except KeyError:
                print("error_key")
                print(candidate)
                continue
        ...
        for item in mbs_set[str(element)].inner_points:
            item_ne = points_set[str(item)].children_info
            for value in item_ne:
                points_set[str(value)].children_info.remove(item)
                points_set[str(value)].children_num -= 1
        ...
        Draw.show_scatter_debug_2(other_points, "blue")     # 蓝点，其他点
        Draw.show_scatter_debug_2(chosen_point_ne, "green")   # 绿点，候选点
        Draw.show_scatter_debug_2(mbs_points, "purple")     # 紫点，基站内部点
        Draw.show_scatter_debug(start_point, "black")       # 黑点，基点
...
def main():
    ...
    __points_set, __points, __adjacency_matrix, __degree, __points_distance, __anti_adjacency_matrix \
        = initialize(400, "normal")  # uniform, normal

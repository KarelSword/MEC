...
def initialize(number):
    ...
    name = 'C:/Users/74412/Desktop/article/Article_Data/Normal/' + str(number) + '_' + str(1) + '.csv'
    ...
    if mbs_count < __k:
        ...
        for j in range(len(ball_order)):
            if j == 0:
                ...
                if radii < opt and saved_radius < opt:
                    mbs_set[str(ball_key)].inner_points.append(chosen_point)
                    mbs_set[str(ball_key)].center = temp_miniball.center()

                    ball_center = saved_ball_center.copy()
                    ball_center[ball_key] = temp_miniball.center()
                    local_optima_radius = saved_radius

                    if radii >= local_optima_radius:
                        local_optima_radius = radii
                    ...
                    if len(uncovered_points) > 0:
                        ...
                        opt, mbs_count, opt_ball_center = \
                            core(recursion_mbs_set, opt, local_optima_radius,
                                 uncovered_points.copy(), copy.copy(ball_center), mbs_count, copy.copy(opt_ball_center))
                    else:
                        opt = local_optima_radius
        ...
        for j in range(__k):
            ...
            if radii < opt and saved_radius < opt:
                ...
                if len(uncovered_points) > 0:
                    ...
                    opt, mbs_count, opt_ball_center = \
                        core(recursion_mbs_set, opt, local_optima_radius, uncovered_points.copy(),
...
if __name__ == "__main__":
    ...
    df.to_csv('C:/Users/74412/Desktop/article/k_center/camp_RDF0424.csv')
    """
    ...
    __optimal, __opt_ball_center = main()
